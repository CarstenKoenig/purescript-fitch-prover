...
_newRuleModal :: Proxy "newRuleModal"
_newRuleModal = Proxy

_assumeNew :: Proxy "assumeNew"
_assumeNew = Proxy

component :: forall q o m. MonadEffect m => H.Component q Problem o m
component =
  H.mkComponent
    { initialState
    , render
...
render state = HH.div_
  ...
  showAssumeNew =
    HH.div
      ...
      , HH.slot _assumeNew unit NewBtn.component unit HandleAssumeNew
...
showRuleButtons scope rules =
  ...
  showRuleButton rule = HH.button
    ...
    , HE.onClick (\_ -> ShowRuleModal rule)
    ]
...
showImplButtons state =
  ...
  showImplButton prem expr = HH.button
    [ HP.class_ (ClassName "button")
    , HE.onClick (\_ -> AddConclusion expr)
    ]
    ...
    case Array.uncons hs of
      ...
      Just { head, tail } ->
        case head of
          (AddedPremisse p) ->
            let
              new = HH.li_ [ HH.span_ [ HH.text "premisse: ", MathJax.showMathJax p.premisse ] ]
            in
              go (Array.snoc acc new) tail
          (UsedRule r) ->
            let
              new = HH.li_
                [ HH.span_
          ...
          (NewAssumption a) ->
            let
              new = HH.li_
                [ HH.span
                    [ HP.classes
                        ( if latestAssumption state == Just head then [ ClassName "latestAssumption" ]
                          else []
                        )
                    ]
            ...
            in
              go (Array.snoc (addSub (Array.snoc acc new)) res.add) res.rest
          (FoundImplication impl) ->
            let
              new = HH.li_ [ HH.span_ [ MathJax.showMathJax impl.newFact ] ]
            in
              { add: new, rest: tail, list: acc }